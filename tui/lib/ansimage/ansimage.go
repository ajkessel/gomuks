...
import (
	...
	badGlobalLog "github.com/rs/zerolog/log" // zerolog-allow-global-log
		...
		for n, row := 0, y; (n <= ai.maxprocs) && (2*row+1 < ai.h); n, row = n+1, y+n {
			go func(row, y int) {
				defer func() {
					err := recover()
					if err != nil {
						evt := badGlobalLog.Error().Bytes("stack", debug.Stack())
						if realErr := err.(error); realErr != nil {
							evt.Err(realErr)
						} else {
							evt.Any(zerolog.ErrorFieldName, err)
						}
						evt.Msg("Panic rendering ANSImage")
